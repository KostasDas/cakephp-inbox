...
class HawkFilesControllerTest extends IntegrationTestCase
{
    ...
    public function testExtremeCase()
    {
        $this->emptyHawkFiles();
        $this->addTestFiles(1000);
        $this->assertTrue($this->checkAllFilesExist());
        // 2000 because files created are for 2 users
        $this->assertEquals($this->checkNumberOfFilesCreated(), 2000);
        $this->assertTrue($this->checkNoFilesUnlinked());

        $data = $this->getFileData(['5']);
        unset($data['hawk_file']);
        $this->editAll($data);
        $this->assertTrue($this->checkAllFilesExist());
        // 1000 because we changed user_id to 5
        $this->assertEquals($this->checkNumberOfFilesCreated(), 1000);
        $this->checkNoFilesUnlinked();

        $data = $this->getFileData(['5']);
        $this->editAll($data, true);
        $this->assertTrue($this->checkAllFilesExist());
        // 1000 because we changed user_id to 5
        $this->assertTrue($this->checkNoFilesUnlinked());
        $this->assertEquals($this->checkNumberOfFilesCreated(), 1000);

        $data = $this->getFileData(['5', '4', '1']);
        $this->editAll($data, true);
        $this->assertTrue($this->checkAllFilesExist());
        // 3000 because files are created for three users
        $this->assertTrue($this->checkNoFilesUnlinked());
        $this->assertEquals($this->checkNumberOfFilesCreated(), 3000);

    }

    private function emptyHawkFiles()
    {
        $config = TableRegistry::exists('HawkFiles') ? [] : ['className' => HawkFilesTable::class];
        $hawkFilesTable = TableRegistry::get('HawkFiles', $config);
        $hawkFilesTable->deleteAll([]);
    }

    private function editAll($data, $files = false)
    {
        $config = TableRegistry::exists('HawkFiles') ? [] : ['className' => HawkFilesTable::class];
        $hawkFilesTable = TableRegistry::get('HawkFiles', $config);

        $ids = $hawkFilesTable->find('list');
        foreach ($ids as $id) {
            if ($files) {
                $data['hawk_file'] = [
                    'tmp_name' => $data['hawk_file']['tmp_name'],
                    'error' => (int) 0,
                    'name' => 'Φ.330_72_1461730_Σ.916_20 ΑΠΡ 2018_ΓΕΣ_ΔΙΔΕΚΠ_'. $id.uniqid() . '.pdf',
                    'type' => 'application/pdf',
                    'size' => (int) 136916
                ];
            }
            $this->post('/hawk-files/edit/'.$id, $data);
        }
    }

    private function checkAllFilesExist()
    {
        $config = TableRegistry::exists('HawkUsers') ? [] : ['className' => HawkUsersTable::class];
        $hawkUsersTable = TableRegistry::get('HawkUsers', $config);

        foreach ($hawkUsersTable->find() as $hawkFile) {
            $file = new File($hawkFile->location);
            // don't include hawk file fixture ids
            if (!$file->exists()) {
                return false;
            }
        }
    ...
    private function checkNumberOfFilesCreated()
    {
        $folder = new Folder(ROOT . DS . 'test_documents');
        return count($folder->tree()[1]);
    }

    private function checkNoFilesUnlinked()
    {
        $folder = new Folder(ROOT . DS . 'test_documents');
        $files = $folder->tree()[1];
        ...
        foreach ($files as $file) {
            $entry = $hawkUsersTable->find()->where(['location' => $file])->first();
            if (empty($entry)) {
                $unlinkedFiles[] = $file;
            }
        }
        return empty($unlinkedFiles);
    }
    ...
    private function addTestFiles($times = 1, $users = [])
    {
        ...
        $this->enableCsrfToken();
        for ($i = 1; $i <= $times; $i++) {
            $data = $this->getFileData($users, $i);
            $this->post('/hawk-files/add.json', $data);
        }
    }

    private function getFileData($userIds = [], $number = 0)
    {
        ...
        $file->create();
        $number = ($number != 0) ? $number : $faker->numberBetween(0, 99999999);
        return [
            ...
            'hawk_file' => [
                ...
                'error' => (int) 0,
                'name' => 'Φ.330_72_1461730_Σ.916_20 ΑΠΡ 2018_ΓΕΣ_ΔΙΔΕΚΠ_'. $number . '.pdf',
                'type' => 'application/pdf',
                'size' => (int) 136916
            ]
        ];
